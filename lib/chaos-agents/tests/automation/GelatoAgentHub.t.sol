...
contract GelatoAgentHub_Test is Test {
  AgentHub public _agentHub;
  GelatoAgentHub public _gelatoAgentHub;
  ...
  function setUp() public {
    ...
    _gelatoAgentHub = new GelatoAgentHub(address(_agentHub));
  ...
  function _checkAndPerformAutomation(uint256 agentId) internal virtual returns (bool) {
    ...
    (bool shouldRunKeeper, bytes memory encodedExecuteData) = _gelatoAgentHub.check(agentIds);
    if (shouldRunKeeper) {
      (bool status, ) = address(_gelatoAgentHub).call(encodedExecuteData);
      assertTrue(status, 'Execute Failed');
    }
